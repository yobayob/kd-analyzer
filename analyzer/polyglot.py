from typing import Tuple, NewType, Awaitable, Callable
...
def _extract_sgml_tag(s: str):
    for a in extract_pipeline(SGML_TAG_PIPELINE, s):
        yield a


def extract_sgml_start(s: str, pos: int) -> MatchPosition:
    match = extract(s, REGEX_EMIT_START_TOKEN, pos=pos)
    return "{}>".format(match.group()), match.end()


def extract_shebang(s: str, pos: int) -> MatchPosition:
    match = extract(s, REGEX_SHEBANG, pos=pos)
    return 'SHEBANG#!{}'.format(parse_shebang(match.group())), match.end()


def extract_sgml(s: str, pos: int) -> MatchPosition:
    match = extract(s, REGEX_SGML, pos=pos)
    return extract_sgml_tokens(match.group()), match.end()


def extract_sgml_tag(s: str, pos: int) -> MatchPosition:
...
def _extract_and_add(r: 're.__Regex') -> Callable[[str, int], MatchPosition]:
    def _extract(s: str, pos: int) -> MatchPosition:
...
def _extract_and_skip_quote(start: 're.__Regex', end: 're.__Regex') -> Callable[[str, int], MatchPosition]:
    def _skip(s: str, pos: int) -> MatchPosition:
...
class Polyglot(Classifier):
    ...
    def classify(self, s: str, guess_class: str = "") ->  str:
        # TODO: fix this ...
        if guess_class in self.MAP:
            return self.MAP[guess_class]
        return super(Polyglot, self).classify(s, guess_class)